...
type SeedKiro struct {
	Name string
	Kiro KiroAccount
}

// SyncAccounts 把 yaml 账号 upsert 进库：身份字段以 yaml 为准，
// 状态字段（enabled/disabled/cooldown/failures）保留库内值。
// upsert-only：库里已有、yaml 没有的账号不删除（管理 API 创建的账号
// 与 yaml 并存；历史用量都在 usage_log）。
func (s *Store) SyncAccounts(seeds []SeedUpstream) error {
	now := time.Now().Unix()
	tx, err := s.db.Begin()
	if err != nil {
		return err
...
// SyncKiroAccounts 把 yaml kiro 账号种子 upsert 进库：凭据身份以 yaml 为准
// （Source/凭据/区域/开关），token_state/冷却/熔断/统计等运行时状态保留库内值
// （轮转后的 refresh_token 以库内值为准）。upsert-only 语义同 SyncAccounts。
func (s *Store) SyncKiroAccounts(seeds []SeedKiro) error {
	now := time.Now().Unix()
	tx, err := s.db.Begin()
	if err != nil {
		return err
	...
	for _, u := range seeds {
		if _, err := tx.Exec(`INSERT INTO accounts
			(name, type, kiro, updated_at)
			VALUES (?,?,?,?)
			ON CONFLICT(name) DO UPDATE SET
				type=excluded.type, kiro=excluded.kiro,
				updated_at=excluded.updated_at`,
			u.Name, TypeKiro, encodeKiro(&u.Kiro), now); err != nil {
			return fmt.Errorf("account: sync kiro %q: %w", u.Name, err)
		}
	}
	return tx.Commit()
}
