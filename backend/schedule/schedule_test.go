...
func (f *fakeUpstream) Report(context.Context, upstreamv1.ResultReport) error { return f.reportErr }
...
func TestReportReturnsUpstreamFailureButStillUpdatesCache(t *testing.T) {
	f := &fakeUpstream{reportErr: errors.New("down")}
	s := newScheduler(t, "sticky", f)
	err := s.Report(context.Background(), "g", upstreamv1.ResolvedTarget{ID: "a/model"}, upstreamv1.ResultReport{ReportID: "r", TargetID: "a/model", Outcome: "abnormal"})
	if err == nil {
		t.Fatal("expected report error")
	}
	g, getErr := s.Store.GroupForModel(context.Background(), "model")
	if getErr != nil || g.LastResult != "abnormal" {
		t.Fatalf("cache=%+v err=%v", g, getErr)
	}
}
