...
func New(base, key string, timeout time.Duration) *Client {
	...
	return &Client{BaseURL: strings.TrimRight(base, "/"), ServiceKey: key, HTTP: &http.Client{Timeout: timeout}, MaxResponse: 2 << 20}
}
...
func (c *Client) Report(ctx context.Context, r upstreamv1.ResultReport) error {
	var out upstreamv1.ResultResponse
	return c.do(ctx, "POST", "/results", r, &out)
}
