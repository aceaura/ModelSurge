...
func TestAuthenticateRedisCacheGated(t *testing.T) {
	...
	// 首次：DB 路径 + 回填缓存
	if configured, ok, _, err := s.Authenticate(ctx, "m1", "anthropic", "k1", ""); err != nil || !configured || !ok {
	...
	if configured, ok, _, err := s.Authenticate(ctx, "m1", "anthropic", "k2", ""); err != nil || !configured || !ok {
	...
	// 协议不匹配（缓存命中分支）
	if configured, ok, _, err := s.Authenticate(ctx, "m1", "openai", "k2", ""); err != nil || !configured || ok {
	...
	if configured, ok, _, err := s.Authenticate(ctx, "m2", "anthropic", "x", ""); err != nil || configured || ok {
	...
	if configured, ok, _, err := s.Authenticate(ctx, "m3", "anthropic", "k", ""); err != nil || configured || ok {
